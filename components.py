...
ComponentContext = {"screen": {}, "position": (0, 0)}
...
class Image(Component):
    ...
    def __init__(self, file: str = "", **kwargs):
        ...
        super().__init__(
            **{"height": self.boundary.height, "width": self.boundary.width, **kwargs}
        )
...
class Row(Component):
    ...
    def __init__(self, gutter: int = 0, direction="right", **kwargs):
        super().__init__(**kwargs)
...
def with_events(component, publisher={}, **kwargs):
    class WithEvents(Wrapper):
        def __init__(
            self, component=component, publisher: Publisher = {}, id=0, **kwargs
        ):
            ...
            for name, handler in self.events.items():
                if name.startswith("on_"):
                    ...
                    if name == EventName.hover:
                        self.listeners[name] = self.publisher.subscribe(
                            pygame.MOUSEMOTION, self.hover(handler)
                        )

                    if name == EventName.click:
                        self.listeners[name] = self.publisher.subscribe(
                            pygame.MOUSEBUTTONUP, self.click(handler)
                        )
...
class Expanded(Row):
    ...
    def draw(self):
        ...
        remaining_space = adjusted_length - sum(
            [getattr(c, direction) for c in sized_children]
        )
...
class Circle(Shape):
    ...
    def draw(self):
        rect = self.get_rect()
        pygame.draw.circle(
            self.context["screen"], self.color, rect.center, rect.width / 2
        )
        super().draw()
        return self
